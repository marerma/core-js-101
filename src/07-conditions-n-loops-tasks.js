...
function findFirstSingleChar(string) {
  ...
  for (let index = 0; index < trimedStr.length; index += 1) {
    const element = trimedStr[index];

    if (trimedStr.filter((el) => el === element).length === 1) {
      newStr.push(element);
    }
  }
  return newStr[0];
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (stack.length !== 0 && openBrackets[topEl] === str[i]) {
      stack.pop();
