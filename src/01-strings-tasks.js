...
function getRectangleString(w, h) {
  const wString = '─';
  const hString = '│';
  const firstString = `┌${wString.repeat(w - 2)}┐\n`;
  const lastString = `└${wString.repeat(w - 2)}┘\n`;
  const middleString = `${hString}${' '.repeat(w - 2)}${hString}\n`;
  const result = [];
  for (let i = 0; i < h; i += 1) {
    if (i === 0) {
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (alphabet.indexOf(letter) === -1) {
      ...
      let index = alphabet.indexOf(letter) + 13;
...
function isString(param) {
  return typeof param === 'string' || Object.prototype.toString.call(param).split(' ')[1].replace(']', '').toLowerCase() === 'string';
}
...
function getCardId(card) {
  ...
  return cards.findIndex((el) => el === card);
}
