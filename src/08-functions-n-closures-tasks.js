...
function logger(func, logFunc) {
  return function hey(...args) {
    const argumentsArr = JSON.stringify([...args]);
    logFunc(`${func.name}(${argumentsArr.slice(1, -1)}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${argumentsArr.slice(1, -1)}) ends`);
    return result;
  };
}
